...
export function MfaEnrollmentView() {
  ...
  // Multi-factor and AAL2 state
  const [verifiedFactors, setVerifiedFactors] = useState<MfaFactorSummary[]>([]);
  const [isAal2, setIsAal2] = useState<boolean>(false);
    ...
    async function checkStatus() {
      try {
        ...
        if (res.hasVerifiedFactor) {
          setVerifiedFactors(res.verifiedFactors || []);
          setIsAal2(res.isAal2 || false);
  ...
  const handleRemoveFactor = (id: string) => {
    ...
    startRemoveTransition(async () => {
      try {
        const res = await unenrollOwnMfaFactorAction(id);
        if (res.success) {
          toast.success("Authenticator factor removed successfully.");
          const status = await getMfaStatusAction();
          if (status.hasVerifiedFactor) {
            setVerifiedFactors(status.verifiedFactors || []);
            setIsAal2(status.isAal2 || false);
            setViewState("ALREADY_ENABLED");
          } else {
            setVerifiedFactors([]);
            setIsAal2(false);
          ...
          setErrorMessage(res.error || "Failed to remove factor.");
        }
      } catch {
        setErrorMessage("An unexpected error occurred while removing factor.");
      }
    });
  };
  ...
  return (
    <div className="p-3.5 sm:p-6 md:p-8 space-y-6 max-w-3xl mx-auto w-full">
      ...
      {viewState === "ALREADY_ENABLED" && (
        <div className="bg-white rounded-2xl border border-emerald-200/80 p-5 sm:p-7 shadow-xs space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">
              Enrolled Authenticators ({verifiedFactors.length})
            </h3>
            {verifiedFactors.map((factor, idx) => (
              <div
                key={factor.id}
                className="bg-slate-50 rounded-xl p-4 border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-violet-100/70 text-violet-700 flex items-center justify-center shrink-0">
                    <Smartphone className="h-4 w-4" />
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-900">
                      {factor.friendlyName || (idx === 0 ? "Primary Authenticator" : "Backup Authenticator")}
                    </div>
                    <div className="text-[11px] text-slate-400">
                      Enrolled: {factor.createdAt ? new Date(factor.createdAt).toLocaleDateString("en-IN") : "Active"}
                    </div>
                  </div>
                </div>

                <div className="flex items-center gap-2 self-end sm:self-center">
                  {factorToRemove === factor.id ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 text-[11px] font-semibold text-slate-600 hover:text-slate-800 transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!isAal2) {
                          setErrorMessage("Removing an active factor requires an AAL2 session. Please log in with your authenticator code first.");
                          return;
                        }
                        setFactorToRemove(factor.id);
                      }}
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-semibold text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
                    >
          ...
          {/* Action buttons: Backup authenticator and back */}
          <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 border-t border-slate-100">
            {verifiedFactors.length < 2 && (
              <button
                ...
                className="inline-flex items-center justify-center gap-2 px-4 py-2.5 min-h-[44px] sm:min-h-[38px] bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition-colors shadow-xs"
              >
                ...
                <span>Add Backup Authenticator</span>
              </button>
            )}
            ...
            <button
              ...
              className="inline-flex items-center justify-center px-4 py-2.5 min-h-[44px] sm:min-h-[38px] bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-colors"
            >
              Back to Settings
            </button>
          </div>
