...
import { recordCommunication } from "@/app/(dashboard)/communications/actions";
...
export function CommunicationCenterView({
  ...
  const countOverdue = initialQueue.filter((i) => i.type === "followup_overdue" || i.type === "doc_expired").length;
  const countToday = initialQueue.filter((i) => i.type === "followup_today").length;
  const countActions = initialQueue.filter((i) => i.type === "request_action").length;
    ...
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
          ...
          <div className="text-2xl font-bold text-slate-900 mt-2">{initialQueue.length}</div>
