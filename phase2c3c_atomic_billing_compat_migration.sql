...
-- MODE: ADDITIVE & COMPATIBLE (Preserves old app direct INSERT behavior temporarily)
--
-- Features included:
-- 1. Idempotency columns, fingerprints & unique indexes on invoices & payments
-- 2. line_position column & index on invoice_items for ordered line items
-- 3. Canonical recalculation function for customer_services.payment_status & sync triggers
-- 4. Internal allocation helper (internal_allocate_payment)
-- 5. Atomic RPCs:
--    - create_invoice_atomic (with JSON ordinality line_position derivation)
--    - record_payment_atomic
--    - record_payment_and_allocate_atomic
...
-- Invoice Items Line Position (ordered items)
...
-- Formula:
--   waived if explicitly waived AND activeInvoiceCount = 0
--   unpaid if activeInvoiceCount = 0 OR totalPaid = 0
--   paid if balanceDue = 0 AND totalPaid > 0
--   partial otherwise (totalPaid > 0 AND balanceDue > 0)
...
BEGIN
  ...
  -- Aggregate active linked invoices
  SELECT 
    COUNT(DISTINCT i.id),
...
-- 3.1 Trigger on invoice_items mutations
...
-- 3.2 Trigger on invoice status and financial changes
...
-- Strictly accessible to postgres / SECURITY DEFINER functions only
-- ==============================================================================
...
BEGIN
  ...
  -- STRICT STATUS VALIDATION: only 'issued' and 'partially_paid' can receive allocations
  ...
  -- Tenant authorization check (if p_user_id provided)
...
-- ==============================================================================
-- 6. PUBLIC RPC: unallocate_payment_atomic (NEW CANONICAL UNALLOCATE RPC)
-- ==============================================================================
...
BEGIN
  ...
  -- Tenant authorization check (bypassed if run by administrative backend)
...
-- Performs atomic, idempotent creation with DB-derived financial calculations
-- Derives invoice_items.line_position from JSON array ordinality
-- ==============================================================================
...
BEGIN
  ...
  -- Compute normalized fingerprint preserving original ordered item sequence
  ...
  -- Validate line items & calculate financial totals in DB
  FOR v_item IN
    SELECT
      x.service_id,
  ...
  LOOP
    ...
    -- Enforce single-request invoice rule across items payload
  ...
  EXCEPTION WHEN unique_violation THEN
    -- Concurrent duplicate race condition: fetch winning record
    IF p_idempotency_key IS NOT NULL THEN
      SELECT id, invoice_number, total_amount, status, idempotency_fingerprint
      INTO v_existing
      FROM public.invoices
  ...
  -- Insert invoice line items deriving line_position from ordinality
  FOR v_item IN
    SELECT
      x.service_id,
      x.customer_service_id,
...
-- Creates a standalone advance/unallocated payment idempotently
-- ==============================================================================
...
-- Creates a payment and immediately allocates it to an invoice atomically
-- ==============================================================================
...
BEGIN
  ...
  -- Perform immediate atomic allocation
  BEGIN
    v_allocation_id := public.internal_allocate_payment(v_payment_id, p_invoice_id, p_amount, v_user_id);
  EXCEPTION WHEN OTHERS THEN
    -- Any allocation failure rolls back the entire transaction including payment insertion
    RAISE;
  END;
...
-- Controlled administrative workflow for setting/unsetting service request waivers
-- ==============================================================================
